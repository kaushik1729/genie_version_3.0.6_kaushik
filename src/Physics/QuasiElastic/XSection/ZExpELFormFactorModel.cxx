...
// from the global parameter list
  ...
  for (int ip=1;ip<fKmax+1;ip++) {
    ...
    alg_key << "QEL-Z_AP" << ip;
    GetParam( alg_key.str(), fZ_APn[ip] ) ;
  }

  // z expansion coefficients
  if (fQ4limit) fZ_BPn = new double [fKmax+5];
  else          fZ_BPn = new double [fKmax+1];

  // load the user-defined coefficient values
  // -- BP0 and BPn for n<fKmax are calculated from other means
  for (int ip=1;ip<fKmax+1;ip++) {
    ostringstream alg_key;
    alg_key << "QEL-Z_BP" << ip;
    GetParam( alg_key.str(), fZ_BPn[ip] ) ;
  }

  // z expansion coefficients
  if (fQ4limit) fZ_ANn = new double [fKmax+5];
  else          fZ_ANn = new double [fKmax+1];

  // load the user-defined coefficient values
  // -- AN0 and ANn for n<fKmax are calculated from other means
  for (int ip=1;ip<fKmax+1;ip++) {
    ostringstream alg_key;
    alg_key << "QEL-Z_AN" << ip;
    GetParam( alg_key.str(), fZ_ANn[ip] ) ;
  }

  // z expansion coefficients
  if (fQ4limit) fZ_BNn = new double [fKmax+5];
  else          fZ_BNn = new double [fKmax+1];

  // load the user-defined coefficient values
  // -- BN0 and BNn for n<fKmax are calculated from other means
  for (int ip=1;ip<fKmax+1;ip++) {
    ostringstream alg_key;
    alg_key << "QEL-Z_BN" << ip;
    GetParam( alg_key.str(), fZ_BNn[ip] ) ;
  }
